<template>
    <div @click="show" class="size-fit">
        <slot name="active">
            <Button label="メディア" class="image" rounded></Button>
        </slot>
    </div>
    ...
    >
        <template #container="{ closeCallback }">
            ...
            <div
                class="w-full h-[60vh] flex flex-col bg-white overflow-y-autorelative"
            >
                <div class="px-5 flex gap-3 flex-wrap">
                    <div v-if="!images.total">画像が見つかりませんでした</div>
                    <Image
                ...
                <div
                    ...
                    v-if="loading"
                >
                    <ProgressSpinner class="z-50" />
                </div>
            </div>

            <div class="flex items-center gap-4 bg-white py-4 px-4 justify-center">
                <Paginator
                    v-if="images.total"
...
const setImagePreviewUrl = async (id) => {
    await api.imageSelector.getImage(id).then(res => {
    ...
    emits('onSelect', {
        value: rValue.value,
        previewImagePath: previewImagePath.value
    })
}
