    ...
    <div class="border-[#bdbdbd] px-[18px] py-[28px] w-full border-l">
        <div class="flex-1">
            <div class="flex justify-left">
                <div class="w-1/4 p-1">
                    <RadioButton :inputId="`select_${inputId}`" :name="`link_type_${inputId}`" :value="1" v-model="formMenu.link_type" />
                    <label class="ml-2" :for="`select_${inputId}`">リストから選ぶ</label>
                </div>
                <div class="w-1/4 p-1">
                    <RadioButton :inputId="`input_${inputId}`" :name="`link_type_${inputId}`" :value="2" v-model="formMenu.link_type" />
                    <label class="ml-2" :for="`input_${inputId}`">直接入力する</label>
                </div>
            </div>
        </div>
...
const props = defineProps(['pages', 'oldInput', 'formData', 'inputId', 'subSpMenus', 'selectedPageType'])
const typeSelect = 1
const typeInput = 2

const inputId = props.inputId
const subSpMenus = props.subSpMenus

const formMenu = reactive({
    input_id: props.inputId,
    link_type: subSpMenus?.link_type,
    link01: subSpMenus?.link01,
    link02: subSpMenus?.link02 ?? '',
    url: subSpMenus?.url ?? '',
    media: subSpMenus?.media ?? '',
    text: subSpMenus?.text ?? '',
})

const listPages = ref([...props.pages])

const selectedPageType = ref(listPages.value.find((value) => {
    return value.key == subSpMenus?.link01
}))
