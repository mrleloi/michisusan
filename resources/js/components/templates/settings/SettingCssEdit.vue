<template>
    <div class="w-full h-full px-3 font-[Inter]">
        <h1 class="text-[22px] font-semibold font-hiragino leading-[33px] mb-3">独自CSS</h1>
      ...
      <span class="text-[#FF0000] text-[14px]">
        ...
        <form :action="props.path" @submit.prevent="onSubmit" method="post">
            ...
            <div class="px-8 py-[28px] w-full border border-[#DFE7EF] mb-8">
                <h1 class="font-semibold font-hiragino text-[22px]">過去のCSS</h1>
                <Select
                ...
                <h1 class="font-semibold font-hiragino text-[22px] mt-8">独自CSS</h1>
...
let dataForm = reactive({
    content: `<pre><code class="language-css">${ props.oldInput?.content ?? lastItem?.content ?? '' }</code></pre>`,
})
...
const fillEditor = () => {
    if (selectedItem.value && selectedItem.value.content) {
        dataForm.content = `<pre><code class="language-css">${selectedItem.value.content}</code></pre>`
    }
}
