...
const props = defineProps(['status', 'path', 'csrfToken', 'errormessage', 'oldInput', 'selectedStatus'])

const status = ref([...props.status])
const selectedStatus = ref(status.value.find((value) => {
    return value.key === (props.oldInput?.status_code || props.selectedStatus)
}))
...
const formData = reactive({
    slug_source: props.oldInput?.slug_source ?? '',
    slug_target: props.oldInput?.slug_target ?? '',
    status_code: props.oldInput?.status_code ?? '',
    redirect_start: props.oldInput?.redirect_start ?? '',
    redirect_end: props.oldInput?.redirect_end ?? '',
})
