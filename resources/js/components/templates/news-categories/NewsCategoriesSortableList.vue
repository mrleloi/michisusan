        ...
        >
            <template #item="{ element }">
                <tr>
                    ...
                    <td v-if="actions.includes('copy')" class="text-center w-48">
                        <a :href="`${items.path}/${element.id}/copy`">
                            <PvButton size="small" label="複製" rounded></PvButton>
                        </a>
                    </td>
                    <td v-if="actions.includes('edit')" class="text-center w-48">
                        <a :href="`${items.path}/${element.id}/edit`">
                            <PvButton size="small" label="編集" rounded></PvButton>
                        </a>
                    </td>
                    <td v-if="actions.includes('delete')" class="text-center w-48">
...
const onDragEnd = async () => {
    ...
    try {
        await api.newsCategories.changeOrder({ items: newOrder })
        errorMessage.value = ''
    } catch (error) {
