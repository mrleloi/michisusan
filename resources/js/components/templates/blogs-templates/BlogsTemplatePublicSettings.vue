<template>
    ...
    <div class="w-full px-14">
        <CheckBox
            v-model="expandForm.is_template"
            binary
            class="flex gap-1"
            inputId="is_template"
            name="is_template"
        >新規テンプレートとして登録</CheckBox>
    </div>
    <div class="border-t w-full my-6"></div>
...
const props = defineProps(['snses', 'categories', 'errorMessage', 'dataUpdate'])
...
const expandForm = reactive({
    publish_status: dataUpdate?.publish_status ?? '',
    published_at: dataUpdate?.published_at ?? '',
    sns_id: dataUpdate?.sns_id ?? [''],
