<template>
    ...
    <div class="w-full px-14">
        <div class="flex items-center border border-[#bdbdbd] bg-[#eff3f8] border-b-0">
            <div class="w-1/3 py-6 flex items-center justify-center font-bold">関連タグ</div>
        ...
        <div class="flex items-center border border-[#bdbdbd] border-b-0">
            <div class="w-1/3 py-6 flex items-center justify-center font-bold">共通フッター</div>
            <div class="w-2/3 p-6 flex items-center border-l border-[#bdbdbd]">
        ...
        <div class="flex items-center border border-[#bdbdbd] bg-[#eff3f8]">
            <div class="w-1/3 py-6 flex items-center justify-center font-bold">アクセス制限</div>
            <div class="w-2/3 p-6 flex flex-col border-l border-[#bdbdbd] gap-y-2">
                <div><label>アカウント：</label>
                    <InputText v-model="expandForm.access_restriction_account" class="w-[117px] h-[38px]" name="access_restriction_account"/>
                </div>
                <span class="text-red-500 text-[12px]" v-if="errorMessage?.account?.length">{{errorMessage?.account?.[0]}}</span>
                <div><label>パスワード：</label>
                    <InputText v-model="expandForm.access_restriction_password" class="w-[117px] h-[38px]" name="access_restriction_password"/>
                </div>
    ...
    <div class="w-full px-14">
        <div class="flex items-center border border-[#bdbdbd] bg-[#eff3f8]">
            <div class="w-1/3 py-6 flex items-center justify-center font-bold">アイキャッチ画像</div>
            <div class="w-2/3 p-6 flex items-center border-l border-[#bdbdbd]">
    ...
    <div class="w-full px-14 pb-11">
        <div class="flex items-center border border-[#bdbdbd] bg-[#eff3f8] border-b-0">
            <div class="w-1/3 py-6 flex items-center justify-center font-bold">ヘッダー</div>
...
const props = defineProps(['subNavigationOptions', 'errorMessage', 'dataUpdate'])
...
const expandForm = reactive({
    show_related_tags: dataUpdate?.show_related_tags ? true : false,
    show_common_footer: dataUpdate?.show_common_footer ? true : false,
    access_restriction_account: dataUpdate?.access_restriction_account ?? '',
    access_restriction_password: dataUpdate?.access_restriction_password ?? '',
    eye_catch: dataUpdate?.eye_catch ?? null,
    show_header: dataUpdate?.show_header ? true : false,
    show_header_logo: dataUpdate?.show_header_logo ? true : false,
    show_header_navigation_menu: dataUpdate?.show_header_navigation_menu ? true : false,
    show_header_mv: dataUpdate?.show_header_mv ? true : false,
    show_footer: dataUpdate?.show_footer ? true : false,
    show_footer_logo: dataUpdate?.show_footer_logo ? true : false,
    show_footer_navigation_menu: dataUpdate?.show_footer_navigation_menu ? true : false,
    show_footer_sns: dataUpdate?.show_footer_sns ? true : false,
