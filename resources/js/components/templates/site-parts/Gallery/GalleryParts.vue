<template>
  <div
    class="w-full flex flex-col justify-center items-center gallery"
    ...
    >
      ...
      <template v-for="category in itemsByCategory" v-bind:key="category.id">
        ...
        <div class="wrapper_item">
          <article
            class="pop"
            v-for="item in category.gallery_items"
            v-bind:key="item.id"
          >
            <div>
              <p class="name">{{ item.title }}</p>
              <p class="subtitle">{{ item.subtitle }}</p>
              <p class="summmary">{{ item.summary }}</p>
              <p class="description">{{ item.description }}</p>
              <div class="inner_item">
                <div class="image" v-if="item.image?.filename">
                  <a href=""
                    ><img
                      :src="`/storage/${item.image?.filename}`"
                      alt="画像1"
                    />
                  </a>
                </div>
