<template>
  <template v-if="tab === ''">
    <h1 class="pl-4 font-bold text-xl">スタッフメンバーデザイン</h1>
  ...
  <template v-if="tab === 'setting'">
    ...
    <div class="px-4 pt-4 pb-4 border-b">
      ...
      <div class="p-8 pt-4 border-b">
        <h1 class="font-bold pb-4">部署・役職</h1>
        <div class="">
          <div class="flex flex-col gap-2">
            <div class="flex flex-row flex-no-wrap items-center">
              <Checkbox
                v-model="attributes.allDepartments"
                inputId="cat_all"
                :binary="true"
                @change="toggleAllDepartment"
              />
            ...
            <div v-for="department in departments" v-bind:key="department.id">
              <Checkbox
                v-model="attributes.departmentIds"
                :inputId="`cat_${department.id}`"
                name="department"
                :value="department.id"
              />
              <label :for="`cat_${department.id}`" class="ml-2">{{
                department.name
              }}</label>
            </div>
          </div>
          <div class="flex flex-wrap gap-x-2 pt-4">
            <Button
              label="全てのカテゴリを解除する"
              @click="attributes.departmentIds = []"
            ></Button>
      ...
      <div class="p-8 pt-4 border-b">
        <h1 class="font-bold pb-4">部署・役職名の表示</h1>
        <div class="flex gap-4">
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showCategoryName"
              inputId="not_show_department_name"
              name="showCategoryName"
              :value="false"
            />
            <label for="not_show_department_name" class="ml-2 cursor-pointer">
              表示しない
            </label>
          </div>
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showCategoryName"
              inputId="show_department_name"
              name="showCategoryName"
              :value="true"
            />
            <label for="show_department_name" class="ml-2 cursor-pointer">
              表示する
            </label>
          </div>
        </div>
      ...
      <div class="p-8 pt-4 border-b">
        <h1 class="font-bold pb-4">自由項目の表示</h1>
        <div class="flex gap-4 flex-wrap">
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showContent"
              inputId="always_show_content"
              name="showContent"
              value="a"
            />
            <label for="always_show_content" class="ml-2 cursor-pointer">
              常に表示
            </label>
          </div>
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showContent"
              inputId="always_fold_content"
              name="showContent"
              value="af"
            />
            <label for="always_fold_content" class="ml-2 cursor-pointer">
              常に折りたたむ
            </label>
          </div>
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showContent"
              inputId="sp_always_fold_content"
              name="showLoadMore"
              value="saf"
            />
            <label for="sp_always_fold_content" class="ml-2 cursor-pointer">
              スマホ時折りたたむ
            </label>
          </div>
        </div>
      ...
      <div class="p-8 pt-4 border-b">
        <h1 class="font-bold pb-4">表示形式</h1>
        <div class="flex gap-4 flex-wrap">
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showType"
              inputId="department_divided_show_type"
              name="showType"
              value="cd"
            />
            <label for="department_divided_show_type" class="ml-2 cursor-pointer">
              カテゴリーで分けて表示
            </label>
          </div>
          <div class="flex items-center">
            <RadioButton
              v-model="attributes.showType"
              inputId="not_department_divided_show_type"
              name="showType"
              value="ncd"
            />
            <label for="not_department_divided_show_type" class="ml-2 cursor-pointer">
              一括表示
            </label>
          </div>
        </div>
...
const toggleAllDepartment = e => {
  if(attributes.value.allDepartments) {
    attributes.value.departmentIds = departments.value.map(i => i.id)
  } else {
...
onMounted(() => {
  ...
  api.staffMemberParts.getDepartments().then((res) => {
