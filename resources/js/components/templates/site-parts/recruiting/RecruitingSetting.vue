<template>
  <template v-if="tab === ''">
    <h1 class="pl-4 font-bold text-xl">リクルートデザイン</h1>
  ...
  <template v-if="tab === 'setting'">
    ...
    <div class="px-4 pt-4 pb-4 border-b">
      ...
      <div class="p-8 pt-4 border-b">
        <h1 class="font-bold pb-4">募集タイトル</h1>
        <div class="flex flex-col gap-y-2">
          <InputText v-model="attributes.title" class="w-full" />
        </div>
      </div>
  ...
  <template v-if="tab === 'category_edit'">
  <iframe src="/admin/recruiting_category" class="w-full h-full"></iframe>
...
onMounted(async () => {
  ...
  api.recruitingParts.getCategories().then(res => {
