<x-manage.container>
    ...
    <x-flash></x-flash>
    <h1 class="text-3xl mb-3">リダイレクト設定</h1>
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('redirects.index') }}">
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('redirects.bulk_delete') }}" method="POST"
            ...
            @else
                <div class="text-center">リダイレクトは登録されておりません</div>
            @endif
        </div>
    </div>
    @if ($items->total())
        ...
        <script>
            function confirmSubmit() {
                ...
                if (checked.length === 0) {
                    ...
                    return confirm('選択したナビゲーションを削除してもよろしいですか?');
                }
            }
        </script>
    @endpush
