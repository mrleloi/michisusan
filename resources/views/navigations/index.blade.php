<x-manage.container>
    ...
    <x-flash></x-flash>
    <h1 class="text-3xl mb-3">サブナビゲーション一覧</h1>
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('navigations.index') }}">
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('navigations.bulk_delete') }}" method="POST"
            ...
            @else
                <div class="text-center">セカンダリナビゲーションが登録されていません</div>
            @endif
        </div>
    </div>
    @if ($items->total())
        ...
        <script>
            function confirmSubmit() {
                ...
                if (checked.length === 0) {
                    ...
                    return confirm('選択したサブナビゲーションを削除してもよろしいですか?');
                }
            }
        </script>
    @endpush
