<x-manage.container>
    ...
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('before_after_category.index') }}">
                <div class="flex flex-wrap gap-y-2">
                    ...
                    <div class="flex-1 justify-end flex items-center whitespace-nowrap">
                        <a href="{{ route('before_after_category.create') }}">
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('before_after_category.bulk_delete') }}" method="POST"
                    ...
                    <sortable-list :items='@json($items)' :columns='@json($columns)'
                        ...
                        disable-check="before_after_items_count"
                        disable-delete="before_after_items_count"
                        :csrf-token='{{ json_encode(csrf_token()) }}'></sortable-list>
                </form>
            @else
                <div class="text-center">ビフォーアフターカテゴリーが見つかりません。</div>
            @endif
        </div>
    </div>
    @if ($items->total())
        ...
        <script>
            function confirmSubmit() {
                ...
                if (checked.length === 0) {
                    ...
                    return confirm('選択したビフォーアフターカテゴリーを削除してよろしいですか？');
                }
            }
        </script>
    @endpush
