<x-manage.container>
    <x-flash></x-flash>
    <h1 class="text-3xl mb-3">スタッフ一覧</h1>
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('staff_member.index') }}">
                <div class="flex flex-wrap gap-y-2">
                    <div class="flex gap-3 items-center flex-wrap">
                        <pv-select size="small" name="department_id" :options='@json($departmentOptions)'
                            option-label="name" option-value="id" :value="{{ request()->query('department_id') ?? null }}"
                            class="w-44" placeholder="組織・役割名一覧" show-clear>
                        </pv-select>
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('staff_member.bulk_delete') }}" method="POST"
            ...
            @else
                <div class="text-center">スタッフが見つかりません。</div>
            @endif
        </div>
    </div>
    @if ($items->total())
