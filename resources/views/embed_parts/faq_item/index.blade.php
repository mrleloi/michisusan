<x-manage.container>
    <x-flash></x-flash>
    <h1 class="text-3xl mb-3">よくある質問一覧</h1>
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('faq_item.index') }}">
                <div class="flex flex-wrap gap-y-2">
                    <div class="flex gap-3 items-center flex-wrap">
                        <pv-select size="small" name="category_id" :options='@json($categoryOptions)'
                            option-label="name" option-value="id" :value="{{ request()->query('category_id') }}"
                            class="w-44" placeholder="カテゴリー一覧" show-clear>
                        </pv-select>
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('faq_item.bulk_delete') }}" method="POST"
                    ...
                    <sortable-list :items='@json($items)' :columns='@json($columns)'
                        ...
                        item-name='選択されたよくある質問'
                        :csrf-token='{{ json_encode(csrf_token()) }}'></sortable-list>
                </form>
            @else
                <div class="text-center">よくある質問が見つかりません。</div>
            @endif
        </div>
    </div>
    @if ($items->total())
        ...
        <script>
            function confirmSubmit() {
                ...
                if (checked.length === 0) {
                    ...
                    return confirm('選択したよくある質問を削除してよろしいですか？');
                }
            }
        </script>
    @endpush
