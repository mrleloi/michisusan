<x-manage.container>
    <x-flash></x-flash>
    <h1 class="text-3xl mb-3">お問い合わせフォーム</h1>
    <div class="border">
        <div class="bg-zinc-200 p-4">
            <form action="{{ route('inquiry_form.index') }}">
                <div class="flex flex-wrap gap-y-2">
                    ...
                    <div class="flex-1 justify-end flex items-center whitespace-nowrap">
                        <a href="{{ route('inquiry_form.create') }}">
        ...
        <div class="p-4">
            @if ($items->total())
                <form name="bulkdelete" action="{{ route('inquiry_form.bulk_delete') }}" method="POST"
                    ...
                    <sortable-list :items='@json($items)' :columns='@json($columns)'
                        ...
                        item-name='選択されたお問い合わせフォーム'
                        :csrf-token='{{ json_encode(csrf_token()) }}'></sortable-list>
                </form>
            @else
                <div class="text-center">お問い合わせフォームが見つかりません。</div>
            @endif
        </div>
    </div>
    @if ($items->total())
